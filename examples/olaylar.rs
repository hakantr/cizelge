    ...
    fn yeni(cx: &mut Context<Self>) -> Self {
        ...
        cx.subscribe(&grafik, |bu: &mut Kök, _, olay: &GrafikOlayı, cx| {
            bu.son_olay = match olay {
                GrafikOlayı::ÖğeTıklandı { seri_adı, ad, değer, .. } => format!(
        ...
        .detach();
        Kök { grafik, son_olay: "Bir dilime, sütuna ya da gösterge öğesine tıklayın".into() }
    }
}
...
fn seçenekler() -> GrafikSeçenekleri {
    GrafikSeçenekleri::yeni()
        ...
        .seri(SütunSerisi::yeni().ad("Satış").veri([5.0, 20.0, 36.0, 10.0, 14.0]))
        .seri(
            PastaSerisi::yeni()
                .ad("Pay")
                .halka("18%", "30%")
