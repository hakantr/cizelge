...
- Profil yalnız kimliği açıkça kayıtlı `matrix-mbti`,
  ...
  `tree-radial`, `tree-vertical`, `custom-profit`, `custom-error-scatter`,
  `custom-bar-trend`, `custom-cartesian-polygon`, `custom-error-bar`,
  `custom-profile`, `cycle-plot`, `custom-polar-heatmap`, `flame-graph`,
  `custom-gauge`, `pie-parliament-transition`, `wind-barb`,
  `custom-gantt-flight`, `circle-packing-with-d3` ve
  `custom-spiral-race` kartlarında açıktır; başka karta otomatik yayılmaz ve
  referans yenileme gerekçesi oluşturmaz. `bar-histogram` ile
  `custom-calendar-icon` ham raster kapısından geçtiği için bu profile dahil
  değildir.
...
#### Custom kategori diliminin uygulanmış durumu

Bu fazın Custom dilimi, kilitli `../echarts/src/chart/custom/CustomSeries.ts`,
`CustomView.ts` ve `customSeriesRegister.ts` kaynakları ile
`../echarts-examples/public/examples/ts` altındaki resmî kartlar esas alınarak
**17/17 kapsam içi kart ve 20/20 kanıt karesi** düzeyinde uygulanmıştır.
`custom-hexbin`, `custom-wind` ve `geo-svg-custom-effect` Geo bağımlılıkları
nedeniyle baştan tanımlanan kapsam dışında kalır; bu sayıların paydasına
girmez.

Taşınan çekirdek yüzey şunları kapsar:

- `ÖzelSeri`, `ÖzelÖğeBağlamı`, `ÖzelKoordinatTanımı`,
  `ÖzelKoordinatHaritası`, `ÖzelÖğeÇıktısı` ve veri diff/geçiş modeli;
  `api.value`, `ordinalRawValue`, `coord`, `size`, `visual`, `style`,
  `barLayout`, `currentSeriesIndices`, görünüm boyutu, action/tur bağlamı ve
  ham/işlenmiş veri sıraları tek tipli Rust API'sinde karşılanır;
- `none`, `cartesian2d`, `polar`, `singleAxis`, `calendar` ve `matrix`
  koordinatları aynı `renderItem` sözleşmesini kullanır. Geo dalı API'de
  sessizce taklit edilmez; kapsam dışı olarak kalır;
- group, rect, circle, ring, sector, arc, line, polyline, polygon, Bézier,
  path/pathData, compound path, text ve image öğeleri; iç içe dönüşüm,
  clipPath, z/z2, opacity, kenarlık, gölge, sessiz öğe, hit bölgesi,
  emphasis/select/blur, `transition` ve `during` sahne ağacında taşınır;
- Custom'ın öntanımlı `clip: false` davranışı korunurken `dataZoom` satırları
  ECharts gibi `encode.x/y` boyutlarında `filter`/`weakFilter` ile
  `renderItem` öncesinde süzer. Açık `encode.x: []` veya `encode.y: []`
  ilgili eksen işlemcisinden bağımsızdır; açık `clip: true` ise koordinat
  alanı kırpmasını ayrıca uygular;
- seri `z` sırası kararlı bağlayıcıyla uygulanır. Custom şekillerinin veya
  sütun dolgularının örttüğü kartezyen taban çizgileri son eksen katmanında
  yeniden çizilir. Kanıt aracı taban çizgisinin uçlarını, sürekliliğini,
  nötrlüğünü ve katmanını ham karede bağımsız kapı olarak denetler;
- zaman ekseninde `minInterval`/`maxInterval` ve sabit saat dilimi, eksen adı
  dönüşü, polar formatter/splitArea ve kategori etiket aralığı tamamlanmıştır.
  Polar kategori otomatik aralığı ölçülen yazı genişliğinden çözülür; açık
  `axisLabel.rotate` bütün etiketleri korur;
- `circle-packing-with-d3`, resmî `option-view.json` verisini ve
  `d3-hierarchy@2.0.0` paketinin `stratify/sum/sort/pack` sonucunu yerel,
  izlenen `examples/uyum_veri/circle-packing-layout.json` varlığına üretir.
  Ağ erişimi kullanılmaz. `custom-spiral-race` kaynak timeout zincirinin
  başlangıç, ilk güncelleme ve son durumunu belirlenimci üç kareye kilitler.

600×450 kilitli raster kapısı genel `pixelmatch=0,1`, değişen piksel `≤%1`
ve `SSIM≥0,99` eşiklerini değiştirmez. Yukarıda açıkça kayıtlı yoğun yazılı
kartlarda iki tarafa aynı `σ=0,8` çekirdeği uygulanır; ham kare ve metrikler
ayrıca saklanır. `custom-profit`, `custom-error-scatter`,
`custom-cartesian-polygon`, `custom-profile`, `cycle-plot`,
`custom-bar-trend` ve `custom-error-bar` için kartezyen taban vuruşları ayrıca
yapısal kapıdan geçmiştir. Özellikle dataZoom penceresinin dışında kalan
trend ve hata öğeleri toplam raster oranına güvenilmeden reddedilir.

| Resmî örnek | Kare | Kapı farkı | Kapı SSIM | Ham fark | Ham SSIM | Durum |
|---|---:|---:|---:|---:|---:|---|
| `bar-histogram` | 1 | %0,1689 | 0,998283 | %0,1689 | 0,998283 | geçti |
| `custom-calendar-icon` | 1 | %0,1126 | 0,998767 | %0,1126 | 0,998767 | geçti |
| `custom-profit` | 1 | %0,0641 | 0,999513 | %0,1463 | 0,999128 | geçti + taban |
| `custom-error-scatter` | 1 | %0,0015 | 0,999512 | %0,1137 | 0,999043 | geçti + taban |
| `custom-bar-trend` | 1 | %0,2648 | 0,995856 | %0,5185 | 0,993003 | geçti + taban |
| `custom-cartesian-polygon` | 1 | %0,1878 | 0,998188 | %0,2848 | 0,997308 | geçti + taban |
| `custom-error-bar` | 1 | %0,2715 | 0,991804 | %0,4815 | 0,990409 | geçti + taban |
| `custom-profile` | 1 | %0,0996 | 0,998314 | %0,3093 | 0,997333 | geçti + taban |
| `cycle-plot` | 1 | %0,0285 | 0,998790 | %0,2926 | 0,998201 | geçti + taban |
| `custom-polar-heatmap` | 1 | %0,0533 | 0,998127 | %0,3474 | 0,997507 | geçti |
| `flame-graph` | 1 | %0,3019 | 0,993739 | %1,4474 | 0,990789 | geçti |
| `custom-gauge` | 1 | %0,4685 | 0,993534 | %0,3596 | 0,992709 | geçti |
| `pie-parliament-transition` | 2 | %0,0000 | 0,999676–0,999901 | %0,0052–0,0296 | 0,999539–0,999854 | geçti |
| `wind-barb` | 1 | %0,1296 | 0,997399 | %0,5841 | 0,995773 | geçti |
| `custom-gantt-flight` | 1 | %0,1619 | 0,994233 | %0,5385 | 0,993468 | geçti |
| `circle-packing-with-d3` | 1 | %0,0796 | 0,993315 | %0,7430 | 0,988544 | geçti |
| `custom-spiral-race` | 3 | %0,0970–0,1493 | 0,993448–0,995302 | %0,2281–0,3170 | 0,991802–0,993730 | geçti |

Fixture ve veri bütünlüğü `examples/uyum_custom.rs`,
`examples/uyum_fixture.rs` ve `tools/uyum/circle_packing_verisi.mjs`; tipli
API `src/model/ozel.rs` ile `src/model/seri.rs`; koordinat, dataZoom, z-sırası
ve eksen katmanı `src/cizim/gorunum.rs`; sahne/durum/isabet yolu
